...
{
public:
	static std::string Int32ToStr(sint32 val)
	{
	...
	static std::string Int64ToStr(sint64 val)
	{
	...
	static std::string UInt32ToStr(uint32 val)
	{
		char buf[16] = { '\0' };
		sprintf(buf, "%u", val);
		buf[15] = 0;
	...
	static std::string UInt64ToStr(uint64 val)
	{
		char buf[32] = { '\0' };
#ifdef _MN_PLATFORM_WIN32_
	...
	static sint32 StrToInt32(const std::string& val)
	{
		return atoi(val.c_str());
	}

	static sint64    StrToInt64(const std::string& val)
	{
#ifdef _MN_PLATFORM_WIN32_
	...
	static float StrToFloat(const std::string& val)
	{
		return (float)atof(val.c_str());
	}

#ifdef _MN_PLATFORM_WIN32_
	static std::wstring StrToWStr(const std::string& val, unsigned int codepage)
	{
		int len;
		...
		AutoMNMem<wchar_t, 512> wsBuf(len);
	...
	static std::string WStrToStr(const std::wstring& val, unsigned int codepage)
	{
		int len;
	...
	static std::wstring    Utf8ToWStr(const std::string& val)
	{
#ifdef _MN_PLATFORM_WIN32_
	...
	static std::string        WStrToUtf8(const std::wstring& val)
	{
#ifdef _MN_PLATFORM_WIN32_
	...
	{
		AutoMNMem<char, 256> auto_mem(str.length() + 1);
	...
	static void    StrReplace(std::string& str, const std::string& old_value, const std::string& new_value)
	{
		while (true)
		{
	...
	static void Split(std::vector<std::string>& result, const std::string & str, const std::string& delims)
	{
		if (0 == str.compare(""))
		{
			return;
	...
	static int SetBit(int & Data, int Pos, int Flag)
	{
		if (Pos < 0 || Pos>31)
			return 0;
