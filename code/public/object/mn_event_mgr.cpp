...
NS_MN_BEGIN
int EventMgr::Invoke(MN_OBJECTID sender, const char* eventname, const VarList& arg)
{
...
void EventMgr::Connect(MN_OBJECTID sender, const char* eventname, EventDelegate recv_callback)
{
...
void EventMgr::Disconnect(MN_OBJECTID sender)
{
	MAP_SENDER::iterator it = m_senders.find(sender);
	if (it == m_senders.end())
		return;
...
void EventMgr::Disconnect(MN_OBJECTID sender, const char* eventname, EventDelegate recv_callback)
{
    MAP_SENDER::iterator it = m_senders.find(sender);
    if(it == m_senders.end())
        return;
