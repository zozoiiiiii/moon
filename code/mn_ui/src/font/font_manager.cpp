...
void FontManager::Init(MN::ObjectMgr* pObjMgr)
{
	m_pObjMgr = pObjMgr;
...
Font* FontManager::GetFont(std::string fontFile, unsigned int fontSize)
{
    ...
    std::map<std::string, Font *>::iterator result = m_fontMap.find(uStr);
	...
	else
	{
        ...
        IRender* pRender = IRender::Instance(m_pObjMgr);
        std::string resPath = pRender->GetResPath();
		std::string strFontFile = resPath + fontFile;
