...
TextComponent::TextComponent():m_atlas(NULL), m_pFont(NULL){}
void TextComponent::OnEvent(const std::string& event, const MN::VarList& args)
{
...
void TextComponent::OnRender(IRender* pRender, RenderContext* pCxt)
{
	...
	MN::Mat4f topLeftMatrix = pRender->GetTopLeftMatrix();
...
void TextComponent::genMesh()
{
	...
	{
        wchar_t c = wideCharStr[i];
		GlyphAtlasNode* gNode = m_atlas->getNode(c);
		...
		Vertex v0;
		v0.pos = mat * MN::Vec3f(0, 0, -1.0f);
		v0.tex_coords = gNode->UV(0.0f, 0.0f);

		Vertex v1;
		v1.pos = mat * MN::Vec3f(w, 0, -1.0f);
		v1.tex_coords = gNode->UV(1.0f, 0.0f);

		Vertex v2;
		v2.pos = mat * MN::Vec3f(w, h, -1.0f);
		v2.tex_coords = gNode->UV(1.0f, 1.0f);

		Vertex v3;
		v3.pos = mat * MN::Vec3f(0, h, -1.0f);
	...
	MN::Vec2f contentSize = MN::Vec2f(result_width, abs(penY) + m_pFont->getFontSize());
...
void TextComponent::initAtlas()
{
	...
	m_atlas->generate();
	m_atlas->generateGLTexture(GetObjectMgr());
}
//NS_MN_END