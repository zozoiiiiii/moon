...
ImageComponent::ImageComponent()
	: m_pTexture(NULL) {}
void ImageComponent::OnEvent(const std::string& event, const MN::VarList& args)
{
...
void ImageComponent::Setup()
{
	...
	m_vertices[0].pos = MN::Vec3f(x1, y1, 0);
	m_vertices[1].pos = MN::Vec3f(x1, y2, 0);
	m_vertices[2].pos = MN::Vec3f(x2, y1, 0);
	m_vertices[3].pos = MN::Vec3f(x2, y2, 0);

	m_vertices[0].tex_coords = MN::Vec2f(0, 0);
	m_vertices[1].tex_coords = MN::Vec2f(0, 1);
	m_vertices[2].tex_coords = MN::Vec2f(1, 0);
	m_vertices[3].tex_coords = MN::Vec2f(1, 1);
...
{
	IRender* pRender = IGUI::Instance(GetObjectMgr())->GetRender();
	if (!pRender)
		return;
