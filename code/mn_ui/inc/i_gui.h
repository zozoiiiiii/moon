...
MN::Rectangle* pRectangle = (MN::Rectangle*)GetMgr()->Create("Rectangle");
...
{
	MN_VIRTUAL_BEGIN(IGUI, BaseObject);
	MN_END
public:
	static IGUI* Instance(ObjectMgr* pObjMgr)
	{
		static IGUI* s_pGUI = NULL;
		if (s_pGUI)
			return s_pGUI;
	...
	virtual MN_OBJECTID GetDesktopForm() const = 0;
	virtual void AddModalForm(MN_OBJECTID control) = 0;
	virtual void RemoveModalForm(MN_OBJECTID control) = 0;
