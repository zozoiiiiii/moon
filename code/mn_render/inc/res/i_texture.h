...
{
	MN_VIRTUAL_BEGIN(ITexture, IRes);
	MN_END
public:
...
{
	MN_VIRTUAL_BEGIN(IStaticTexture, ITexture);
	MN_END
public:
...
{
	MN_VIRTUAL_BEGIN(IDynamicTexture, ITexture);
	MN_END
public:
    virtual void Generate(int w, int h, int n, const void* pData) = 0;
};
...
{
    MN_VIRTUAL_BEGIN(IRenderTexture, ITexture);
    MN_END
};

class IFontTexture : public ITexture
{
	MN_VIRTUAL_BEGIN(IFontTexture, ITexture);
	MN_END
public:
	virtual void Generate(int w, int h, int n, const void* pData) = 0;
};