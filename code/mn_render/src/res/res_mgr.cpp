...
#include "external/texture/stbi_DDS_aug.h"


IRes* ResMgr::CreateRes(const char* name)
{
    BaseObject* pBaseObject = GetObjectMgr()->create(name);
	if(!pBaseObject->IsInstanceOf("IRes"))
        return NULL;
...
ITexture* ResMgr::InnerLoadTexture(const std::string& file)
{
    ...
    IStaticTexture* pRes = (IStaticTexture*)GetObjectMgr()->create("StaticTexture");
    pRes->SetResMgr(this);
	pRes->SetWidth(x);
	pRes->SetHeight(y);
...
IShader* ResMgr::LoadShader(const std::string& vsh, const std::string& fsh)
{
    ...
    IShader* pShader = (IShader*)GetObjectMgr()->create("Shader");
