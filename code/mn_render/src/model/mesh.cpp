...
#include "mn_render/inc/res/i_res_mgr.h"
...
bool Mesh::LoadFromFile(const std::string& file)
{
    ...
    for (int i = 0; i < SubMesh_num; i++)
    {
        ...
        for (int j = 0; j < vert_num; j++)
        {
            ...
            if(vertex_index != j)//, "vertex index:"<<vertex_index<<" vs "<<j);
                return false;
        ...
        for (int j = 0; j < numFaces; j++)
        {
            ...
            if(face_index != j)//, "face index:"<<face_index<<" vs "<<j);
                return false;

            if(numIndices != 3)//, "every face should has 3 indices:"<<numIndices);
                return false;
...
void Mesh::CalNormalLines(SubMesh* pSM)
{
    ...
    for (unsigned int i = 0 ; i < indices_count ; i += 3)
	{
        ...
        Vertex* v0 = &pSM->vertices[Index0];
        Vertex* v1 = &pSM->vertices[Index1];
        Vertex* v2 = &pSM->vertices[Index2];

        Vec3f v10 = v1->position - v0->position;
        Vec3f v20 = v2->position - v0->position;

        // normal = v1 cross v2
        Vec3f Normal = v10.cross(v20);
        Normal.norm();
        v0->normal += Normal;
        v1->normal += Normal;
        v2->normal += Normal;
