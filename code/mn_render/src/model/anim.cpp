...
void Anim::Load(Skeleton* skel, const std::string& file)
{
    m_skel = skel;
    load(file);
...
bool Anim::load(const std::string& file)
{
...
void Anim::render(IShader* sr)
{
    ...
    if(u_hasAnimation < 0)//;;, "no uniform called:u_hasAnimation in Shader:"<< ph);
        return;
    ...
    for(int i=0; i< bone_num; i++)
    {
        ...
        if(g_bones < 0)//, "no uniform called:u_hasAnimation in Shader:" << ph);
            continue;
